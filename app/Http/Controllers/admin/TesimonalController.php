...
class TesimonalController extends Controller
{
    ...
    public function index()
    {
        ...
        $setting = Testimonal::whereNotNull('heading')
            ->latest()
            ->first();
        return view('admin.testimonal.index', ['test' => $test, 'setting' => $setting]);
    }
    ...
    {
        Testimonal::create([
            'count' => $request->happy_customer_count,
            'heading' => $request->testimonial_heading,
        ]);
    ...
    {
        $setting = Testimonal::findOrFail(decrypt($id));
