<section class="w-full my-16">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-7 items-stretch">
        ...
        {{-- IMAGE COLUMN --}}
        @if ($imageLayout)
            <div class="{{ $imageOrder }} max-h-[70vh] lg:h-auto">
                <img src="{{ asset('storage/' . $imageLayout->image) }}"
                     class="w-full h-full object-cover rounded-md min-h-[400px]">
                {{-- object-cover prevents stretching while h-full fills the content height --}}
            </div>
        @endif
        ...
        @if ($contentLayout)
            <div class="{{ $contentOrder }} flex flex-col justify-start px-4 lg:px-6">
                ...
                @if ($contentLayout->heading)
                    <h2 class="text-3xl md:text-[40px] md:leading-[50px] font-inter" style="color: {{ $contentLayout->heading_color ?? '#000000' }};">
                        {{ $contentLayout->heading }}
                    </h2>
                @endif

                @if ($contentLayout->subheading)
                    <h2 class="text-xl md:text-[20px] md:leading-[30px] font-inter mt-2" style="color: {{ $contentLayout->subheading_color ?? '#000000' }};">
                        {{ $contentLayout->subheading }}
                    </h2>
                @endif

                @if ($contentLayout->paragraph)
                    <p class="text-base text-gray-600 font-inter my-7" style="color: {{ $contentLayout->text_colors ?? '#000000' }};">
