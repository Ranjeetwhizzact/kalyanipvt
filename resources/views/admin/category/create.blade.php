...
<body class="bg-gray-100">
    ...
    <div class="main-content flex-1 p-6 ml-64 transition-all duration-300">
      ...
      <div class="bg-white p-6 rounded-lg shadow-md max-w-6xl mx-auto mt-6">
        ...
        <div class="flex justify-between items-center mb-6 border-b pb-4">
          ...
          <a href="{{ url('category-list') }}" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition-colors flex items-center gap-2">
        ...
        <div class="bg-green-100 border-l-4 border-green-500 text-green-700 px-4 py-3 rounded relative mb-6 flex items-center" role="alert">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
              ...
              <div class="flex items-center space-x-4">
                ...
                <div class="flex-1">
                  ...
                  <label for="fileInput" class="bg-blue-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-600 cursor-pointer inline-block">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
              ...
              <div class="flex items-center space-x-4">
                ...
                <div class="flex-1">
                  ...
                  <label for="fileInputicon" class="bg-blue-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-600 cursor-pointer inline-block">
                    <i class="ri-upload-line mr-1"></i> Choose Icon
                  </label>
                  <input type="file" name="icon" id="fileInputicon" class="hidden" accept="image/*"
                         {{ !isset($category->id) ? 'required' : '' }}>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label for="name" class="block text-sm font-medium text-gray-700 mb-2">
                <i class="ri-price-tag-3-line mr-1"></i>Category Name <span class="text-red-500">*</span>
              </label>
              <input type="text" 
                     id="name" 
                     name="name" 
            ...
            <div>
              ...
              <select id="status" 
                      ...
                      required>
                <option value="" disabled {{ !isset($category->is_active) ? 'selected' : '' }}>Select category status</option>
                <option value="active" {{ (old('is_active', isset($category->is_active) ? $category->is_active : '') == 'active') ? 'selected' : '' }}>Active</option>
                <option value="inactive" {{ (old('is_active', isset($category->is_active) ? $category->is_active : '') == 'inactive') ? 'selected' : '' }}>Inactive</option>
              </select>
              @error('is_active')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
              @enderror
            ...
            <div class="md:col-span-2">
              <label for="short_discription" class="block text-sm font-medium text-gray-700 mb-2">
                <i class="ri-file-text-line mr-1"></i>Short Description <span class="text-red-500">*</span>
              </label>
              <textarea id="short_discription" 
                        name="short_discription" 
                        rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Enter a brief description of the category"
                        required>{{ old('short_discription', isset($category->short_discription) ? $category->short_discription : '') }}</textarea>
            ...
            <div class="md:col-span-2">
              <label for="discription" class="block text-sm font-medium text-gray-700 mb-2">
                <i class="ri-file-copy-line mr-1"></i>Full Description <span class="text-red-500">*</span>
              </label>
              <textarea id="discription" 
                        ...
                        required>{{ old('discription', isset($category->discription) ? $category->discription : '') }}</textarea>
              @error('discription')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
              @enderror
            </div>
  ...
  <script>
    ...
    document.querySelector('button[type="reset"]').addEventListener('click', function(e) {
      ...
      @if(isset($category->id))
        document.getElementById('selectedImage').src = "{{ isset($category->img) && $category->img ? asset('storage/'.$category->img) : asset('backend/assests/img/default-category.png') }}";
        document.getElementById('selectedImageicon').src = "{{ isset($category->icon) && $category->icon ? asset('storage/'.$category->icon) : asset('backend/assests/img/default-icon.png') }}";
      @else
        document.getElementById('selectedImage').src = "{{ asset('backend/assests/img/default-category.png') }}";
        document.getElementById('selectedImageicon').src = "{{ asset('backend/assests/img/default-icon.png') }}";
      @endif
