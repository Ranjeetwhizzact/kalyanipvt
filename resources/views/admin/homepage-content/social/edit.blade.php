        ...
        <div class="main-content flex-1 p-6 ml-64 transition-all duration-300">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md max-w-4xl mx-auto mt-10">
                ...
                <form action="{{ route('admin.social.update', encrypt($social->id)) }}" method="POST"
                    ...
                    <!-- Current Icon -->
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2">Current Icon</label>

                        @if ($social->icon)
                            <div class="mb-3">
                                <img src="{{ asset($social->icon) }}" class="w-24 h-24 object-cover rounded border">
                            </div>
                        @else
                            <p class="text-gray-500 text-sm">No icon uploaded</p>
                        @endif

                        <label class="block text-sm font-medium mb-2 mt-3">Replace Icon</label>
                        <input type="file" name="icon" id="icon" class="w-full border p-2 rounded"
                            accept="image/*">

                        <!-- Preview New Icon -->
                        <div
                            class="w-24 h-24 border mt-3 rounded bg-gray-100 flex items-center justify-center overflow-hidden">
