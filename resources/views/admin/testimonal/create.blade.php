    ...
    <div class="flex h-screen">
        ...
        <div class="main-content flex-1 p-6 ml-64 transition-all duration-300">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md max-w-4xl mx-auto mt-10">
                ...
                @endif
                <h2 class="text-2xl font-semibold mb-6">Add New Testimonal</h2>
                <form action="{{ url('storetest') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <!-- Row for Input Fields: Name, Category, Image -->
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <input type="hidden" name="id" value="{{ isset($test->id) ? $test->id : '' }}">
                        <!-- Name Input -->
                        ...
                        @if (isset($test->id))
                            <div class="col-span-2">
                                <label for="fileInput" class="block text-sm font-medium text-gray-700 mb-2">Upload
                                    photo</label>
                                <label for="fileInput">
                                    <img src="{{ $test->image ?? asset('backend/assests/img/article2.png') }}"
                        ...
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <input type="text" id="title" name="name"
                                value="{{ isset($test->name) ? $test->name : '' }}"
                        ...
                        <div class="col-span-2 md:col-span-1">
                            <label for="occupation"
                                class="block text-sm font-medium text-gray-700 mb-2">Occupation</label>
                            <input type="text" id="title" name="occupation"
                                value="{{ isset($test->occupation) ? $test->occupation : '' }}"
                                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div class="mb-4 col-span-2 md:col-span-1">
                            ...
                            <select id="status" name="is_active"
                                value="{{ isset($test->is_active) ? $test->is_active : '' }}"
                                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                required>
                                <option value="" disabled selected>Select product status</option>
                                <option
                                    {{ isset($test->is_active) && $test->is_active == 'active' ? 'selected' : '' }}>
                                    Active</option>
                                <option
                                    {{ isset($test->is_active) && $test->is_active == 'inactive' ? 'selected' : '' }}>
                                    Inactive
                                </option>
                            </select>
                        </div>
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <input type="date" id="title" name="date"
                                value="{{ isset($test->date) ? $test->date : '' }}"
                                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Enter occupation" required />
                        </div>
                        ...
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <textarea id="message" name="message" value=""
                                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Enter message description" rows="7" required>{{ isset($test->message) ? $test->message : '' }}</textarea>
                        </div>
