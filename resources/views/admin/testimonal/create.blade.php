        ...
        <form action="{{url('storetest')}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div class="col-span-2">
              ...
              <label for="fileInput">
                <img src="{{ $test->image ?? asset('backend/assests/img/article2.png') }}" alt="Selected Image"
            ...
            <div class="col-span-2 md:col-span-1">
              ...
              <input type="text" id="title" name="name" value="{{isset($test->name)?$test->name:''}}"
            ...
            <div class="col-span-2 md:col-span-1">
              ...
              <input type="text" id="title" name="occupation" value="{{isset($test->occupation)?$test->occupation:''}}"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div class="mb-4 col-span-2 md:col-span-1">
              ...
              <select id="status" name="is_active" value="{{isset($test->is_active)?$test->is_active:''}}"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                required>
                <option value="" disabled selected>Select product status</option>
                <option {{ isset($test->is_active) && $test->is_active == "active" ? 'selected' : '' }}>Active</option>
                <option {{ isset($test->is_active) && $test->is_active == "inactive" ? 'selected' : '' }}>Inactive
                </option>
              </select>
            </div>
            <div class="col-span-2 md:col-span-1">
              <label for="occupation" class="block text-sm font-medium text-gray-700 mb-2">date</label>
              <input type="date" id="title" name="date" value="{{isset($test->date)?$test->date:''}}"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Enter occupation" required />
            </div>
